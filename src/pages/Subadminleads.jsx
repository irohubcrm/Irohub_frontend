...
import { toggleCustomerdetailmodal, toggleCustomermodal, toggleUploadcsvmodal } from '../redux/modalSlice';
import Customermodal from '../components/Customermodal';
import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query';
import Customerdetailmodal from '../components/Customerdetailmodal';
import { AnimatePresence, motion } from 'framer-motion';
import { deletemultipleleads, listleads, updateleadstatus, updatepriority } from '../services/leadsRouter';
...
// Sorting leads by statusOrder
    ...
    const iscustomerdetailmodal = useSelector((state) => state.modal.customerdetailModal);
    ...
    const handleleadchange = async (leadId, status) => {
        ...
        setTimeout(() => {  setstatussuccessmodal(false);
        }, 2000);
    };
                ...
                <div className="flex flex-col min-h-screen">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 sm:p-6 bg-white shadow sticky top-0 z-30 border-b">
                        <div className="flex items-center mb-4 sm:mb-0">
                            ...
                            <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900">Leads</h3>
                        </div>
...
{/* there is changes for me */}
              ...
              >
                ...
                <option value="Not Assigned">Not Assigned</option>
                            ...
                            {datefilter === 'custom' && (
                                <input
                                    type="date"
                                    onChange={(e) => setselectdatefilter(new Date(e.target.value))}
                                                    ...
                                                    <th className="py-2 sm:py-3 px-2 sm:px-4 text-left">
                                                        {role !== 'Agent' && (
                                                            <input
                                                                type="checkbox"
                                                                checked={
                                                                    selectedLeads.length === fetchleads?.leads?.length &&
                                                                    fetchleads?.leads?.length > 0
                                                                }
                                                                onChange={(e) => {
                                                                    if (e.target.checked) {
                                                    ...
                                                    >
                                                        ...
                                                        <td className="py-2 sm:py-4 px-2 sm:px-4 text-gray-600 align-middle">
                                                            {lead.assignedTo?.name || (
                                                                <span className="italic text-gray-400">Not yet assigned</span>
                                                            )}
                                                        </td>
                                                        <td className="py-2 sm:py-4 px-2 sm:px-4 align-middle">
                                                            {lead.createdAt
     ...
     <select
                              ...
                              >
                                <option value="hot" className=''>Hot</option>
                                <option value="warm">Warm</option>
                                <option value="cold">Cold</option>
                                <option value="Not Assigned">Not Assigned</option>
                              </select>
